/*
 * invert.c - Create inverted index from bibliographic database
 */
...
static long int shorten(char *inf, char *outf);
...
*/
static long int
shorten(char *inf, char *outf)
{
    FILE *in, *out;
    char line[maxstr];
    char key[maxstr], newkey[maxstr];
    char file[maxstr], newfile[maxstr];
    long int start, length;
    long int lines = 0;
    ...
    for (mygetline(in, line); !feof(in); mygetline(in, line)) {
